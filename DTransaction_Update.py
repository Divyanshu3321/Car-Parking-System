...
def TrandactionUpdate():
    root = tk.Toplevel()

    root.title("Update Trndaction Records")
    def update_record():  ##function ka andr function banana sa we can use previous values
        Old_Transaction_ID = l9.get()
        Updated_Transaction_ID = l11.get()

        if len(Old_Transaction_ID) == 0 or len(Updated_Transaction_ID) == 0:
            mssg = "You haven't filled all the required details."
            l14.config(text=mssg)
        else:
            query1 = f"UPDATE transaction SET Transaction_ID = '{Updated_Transaction_ID}' WHERE Transaction_ID = '{Old_Transaction_ID}';"
            query2 = f"UPDATE customer SET Transaction_ID = '{Updated_Transaction_ID}' WHERE Transaction_ID = '{Old_Transaction_ID}';"
    ...
    l8 = ttk.Label(root, text="Enter Your Existing Transaction ID")
    l8.grid(row=1, column=0, padx=10, pady=10)
    l9 = ttk.Entry(root)
    l9.grid(row=1, column=1, padx=10, pady=10)

    l10 = ttk.Label(root, text="Enter Updated Transaction ID ")
